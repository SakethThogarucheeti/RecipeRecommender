<!-- This php file contains the code for the UserDetails  registration process. 
The access to this page will be restricted for signed in UserDetails . 
...
if (isset($_POST['signup'])) {
    $uname = mysqli_real_escape_string($con, $_POST['uname']); // UserDetails  name
    ...
    if(!preg_match("/^[A-Za-z][A-Za-z0-9]*(?:_[A-Za-z0-9]+)*$/",$uname) || strlen($uname)<3 || strlen($uname)>15){
        $error = true;
        $uname_error = "UserDetails  Name must start with alphabet character, and contain only 3 -15 characters (only alphanumeric or the underscore characters)";//how to check whether UserDetails name already registered, case sensitive
    }
    if(!isset($uname_error)) { //valid uname, check repeation 
    	...
    	if($result = mysqli_query($con, $query)){
    		$row = mysqli_fetch_array($result);
   	 		if(isset($row)) {
    			$error = true;
        		$sameUname = "That name is already regisered. Do you want to log in?";
    		} 
    	} 
    }
    ...
    if(!isset($uemail_error)) { //valid uemail, check repeation 
    	$query = "SELECT * FROM UserDetails  WHERE `uemail` ="." '" . $uemail . "'";
    	if($result = mysqli_query($con, $query)){
    ...
    if (!$error) {
//     	echo "INSERT INTO UserDetails (uname, oname, upassword, uemail) VALUES('" . $uname . "', '" . $oname . "', '" . md5(md5($uname).$upassword) . "', '" . $uemail . "')";
    	// $query = INSERT INTO `UserDetails `(`uname`, `oname`, `upassword`, `uemail`) VALUES('" . $uname . "', '" . $oname . "', '" . $upassword . "', '" . $uemail . "')";
        // $quert = "UPDATE `UserDetails ` SET `uemail` = 'ian@hotmail.com' WHERE `uname` = 'Ian' LIMIT 1"; //set all the emails if no WHERE, limit one even more than one are set, if no where, at most lose one email
        // mysqli_query($link, $query);
        if(mysqli_query($con, "INSERT INTO UserDetails (uname, oname, upassword, uemail) VALUES('" . $uname . "', '" . $oname . "', '" . md5(md5($uname).$upassword) . "', '" . $uemail . "')")) {
